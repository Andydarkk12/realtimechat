...
export const Messanges = ({changeChatName, changeChatImage, userId, setEditing, editing, kickUser, chatMembers, getMembers, choosedChat, messanges, sendMessage }) => {
  ...
  return (
    ...
    <div className="flex-1 flex flex-col bg-white">
      ...
      {editing ? <EditChat changeChatImage={changeChatImage} changeChatName={changeChatName} userId={userId} choosedChat={choosedChat} kickUser={kickUser} chatMembers={chatMembers}/> : ''}
      {/* Messages block */}
      <div className="flex-1 p-4 overflow-y-auto space-y-2">
        {messanges.map((msg) => (
          <MessangesDisplay chatMembers={chatMembers} key={msg.id} messange={msg} />
        ))}
      </div>
